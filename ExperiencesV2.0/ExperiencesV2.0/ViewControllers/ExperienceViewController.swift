...
class UserExperienceViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    private let titleTextField: UITextField = {
        ...
        textField.placeholder = "Enter Experience Title"
    ...
    lazy private var imageView: UIImageView = {
        let view = UIImageView(image: UIImage(named: "default"))
    ...
    private func setUpNavigationBar() {
        ...
        navigationItem.title = "Add Experience"
    ...
    private func filterPhotoEffectNoir(_ image: UIImage) -> UIImage? {
    ...
    private func filtercolorPosterize(_ image: UIImage) -> UIImage? {
        guard let cgImage = image.cgImage else { return nil }
    ...
    @objc func segmentSwitch(sender: UISegmentedControl) {
        ...
        case 1:
            ...
            imageView.image = filtercolorPosterize(imageToFiler)
            
        case 2:
            guard let imageToFiler = scaledImage else { return }
            imageView.image = filterPhotoEffectNoir(imageToFiler)
        default:
            break
        }
    }
    ...
    @objc func handleNext() {
        dismiss(animated: true, completion: nil)
        let item = Experience(postTitle: titleTextField.text!, postURL: nil, latitude:MapViewController.locationManager.location!.coordinate.latitude , longitude: MapViewController.locationManager.location!.coordinate.longitude)
        items.append(item)
...
extension UserExperienceViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func requestPermissionAndShowCamera()  {
        ...
        case .notDetermined:
            requestPermission()
        case .restricted:
            fatalError("You don't have permission to use the camera")
        case .denied:
    ...
    func requestPermission() {
        AVCaptureDevice.requestAccess(for: .video) { granted in
            guard granted else { fatalError("Video permission is necessary") }
